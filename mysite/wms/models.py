...
class Column(ComputedFieldsModel):
    ...
    for_buffer = models.ForeignKey(
        Buffer, on_delete=models.SET_NULL, blank=True, null=True)
...
class Storage(ComputedFieldsModel):
    storage_id = models.CharField(
        max_length=10, primary_key=True, unique=True, verbose_name=_('Storage ID'))
    layout_col = models.PositiveSmallIntegerField(
        default=1, verbose_name=_('Layout Col'))
    layout_row = models.PositiveSmallIntegerField(
        default=1, verbose_name=_('Layout Row'))
    column_id = models.ForeignKey(
        Column, on_delete=models.SET_NULL, blank=True, null=True, verbose_name=_('Column ID'))
    coor_id = models.ForeignKey(Coordinate, on_delete=models.SET_NULL,
                                blank=True, null=True, verbose_name=_('Coordinate ID'))
    inv_product = models.ForeignKey(
        Product, on_delete=models.SET_NULL, blank=True, null=True, verbose_name=_('Inventory Product'))
    inv_qty = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_('Inventory Quantity (Bag)'))
    lot_name = models.CharField(
        max_length=100, blank=True, null=True, verbose_name=_('Lot Name'))
    created_on = models.DateTimeField(
        blank=True, null=True, verbose_name=_('Created On'))
    updated_on = models.DateTimeField(
        blank=True, null=True, verbose_name=_('Updated On'))
    history = HistoricalRecords(excluded_fields=['layout_col', 'layout_row', 'column_id', 'coor_id', 'lot_name', 'updated_on',
                                                 'zone', 'area', 'col', 'row', 'coor_x', 'coor_y', 'name_eng', 'bg_color', 'font_color'])
    ...
    def save(self, *args, **kwargs):
        try:
            self.column_id = Column.objects.get(
                column_id=str(self.storage_id)[0:6])
        except:
            self.column_id = None
        try:
            self.coor_id = Coordinate.objects.get(
                layout_col=self.layout_col, layout_row=self.layout_row)
        except:
            self.coor_id = None
        super().save(*args, **kwargs)
...
class AgvProductionPlan(ComputedFieldsModel):
    product_name = models.ForeignKey(
        Product, on_delete=models.CASCADE, blank=True, null=True, verbose_name=_('Plan Product'))
    qty_total = models.PositiveIntegerField(
        verbose_name=_('Total Quantity (Bag)'))
    qty_remain = models.PositiveIntegerField(
        verbose_name=_('Remaining Quantity (Bag)'))
    lot_name = models.CharField(
        max_length=100, blank=True, null=True, verbose_name=_('Lot Name'))
...
class RobotStatus(ComputedFieldsModel):
    robot_no = models.PositiveSmallIntegerField(
        primary_key=True, unique=True, verbose_name=_('Robot No.'))
    brand = models.PositiveSmallIntegerField(
        blank=True, null=True, default=1, verbose_name=_('Brand No.'))
    qty_act = models.PositiveIntegerField(
        blank=True, null=True, default=1, verbose_name=_('Actual Quantity (Bag)'))
    qty_target = models.PositiveIntegerField(
        blank=True, null=True, default=1, verbose_name=_('Target Quantity (Bag)'))
...
class RobotQueue(ComputedFieldsModel):
    ...
    robot_no = models.PositiveSmallIntegerField(
        choices=robot_choices, verbose_name=_('Robot No.'))
    ...
    product_id = models.PositiveSmallIntegerField(
        choices=product_id_choices, blank=True, null=True, verbose_name=_('Product ID'))
    qty_act = models.PositiveIntegerField(
        verbose_name=_('Actual Quantity (Bag)'))
    updated_choices = [(0, _('Wait')), (1, _('Ready'))]
    updated = models.PositiveSmallIntegerField(
        choices=updated_choices, verbose_name=_('Updated'))
    history = HistoricalRecords()
...
class AgvQueue(ComputedFieldsModel):
    product_name = models.ForeignKey(
        Product, on_delete=models.CASCADE, blank=True, null=True, verbose_name=_('Queuing Product'))
    lot_name = models.CharField(
        max_length=100, blank=True, null=True, verbose_name=_('Lot Name'))
    qty_act = models.PositiveIntegerField(
        verbose_name=_('Actual Quantity (Bag)'))
    ...
    robot_no = models.PositiveSmallIntegerField(
        choices=robot_choices, blank=True, null=True, verbose_name=_('Robot No.'))
    pick_id = models.ForeignKey(Storage, on_delete=models.CASCADE, blank=True,
                                null=True, verbose_name=_('Pick ID'), related_name='pick_id')
    place_id = models.ForeignKey(Storage, on_delete=models.CASCADE, blank=True,
                                 null=True, verbose_name=_('Place ID'), related_name='place_id')
    mode_choices = list(
        enumerate([_('Storage'), _('Retrieval/Move')], start=1))
    mode = models.PositiveSmallIntegerField(
        choices=mode_choices, verbose_name=_('Mode'))
    history = HistoricalRecords()
...
class AgvTransfer(ComputedFieldsModel):
    ...
    status_choices = list(
        enumerate([_('Wait for queue'), _('Sending command'), _('Operating queue')]))
    status = models.PositiveSmallIntegerField(
        choices=status_choices, default=1, verbose_name=_('AGV Status'))
    ...
    pattern_choices = [(0.0, 'P0: ArmRun -> Rev'), (1.0, 'P1: Rev -> ArmRun'), (2.0, 'P2: ArmPrepare -> FW -> Pick(Robot)'),
                       (3.0, 'P3: ArmPrepare -> FW -> Pick(Storage)'), (4.0, 'P4: FW -> ArmPut')]
    ...
    history = HistoricalRecords(excluded_fields=[
                                'x_nav', 'y_nav', 'beta_nav', 'x1', 'y1', 'x2', 'y2', 'x3', 'y3', 'x4', 'y4', 'x5', 'y5'])
